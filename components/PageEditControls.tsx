...
import { Languages, Eraser } from 'lucide-react'
...
export function PageLyricsActions({ entry, onReplace }: {
  ...
  return <>
    <Button variant="outline" size="sm" className="text-xs" disabled={converting || empty} onClick={convert}>
      <Languages className="h-3 w-3 mr-1" />{converting ? '変換中…' : 'ひらがな'}
    </Button>
    <Button variant="outline" size="sm" className="text-xs" disabled={empty} onClick={() => onReplace(entry.id, ['', '', '', ''])}>
      <Eraser className="h-3 w-3 mr-1" />クリア
    </Button>
  </>
}
